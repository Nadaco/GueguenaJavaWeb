...
<h1><%= isListeComplete ? "Les absences de tous les groupes" : " Absences des " + etudiants.get(0).getGroupe().getNom() %>
</h1>

<!-- Lien vers les autres groupes-->
<% for (Groupe groupe : groupes) {%>
<% if (etudiants.get(0).getGroupe().getId() != groupe.getId() || isListeComplete == true) {%>
<a href="<%= application.getContextPath()%>/do/absencesetudiants?idGroupe=<%=groupe.getId()%>">
    <%=groupe.getNom()%>
</a> &nbsp;
<% } %>
<% } %>
...
<a href="<%= application.getContextPath()%>/do/absencesetudiants">Tous les groupes</a>
<% }%>
